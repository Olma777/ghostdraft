# Pester 5 — install.ps1 (Windows-порт ghostdraft). Проверяем integrity-gate без сети:
# GHOSTDRAFT_BASE_URL указывает на локальный каталог-«релиз», установка идёт во временный
...
Describe 'install.ps1 integrity gate' {
    ...
    It 'installs the script when the checksum matches' {
        & pwsh -NoProfile -Command "`$env:GHOSTDRAFT_BASE_URL='$($script:Release)'; `$env:GHOSTDRAFT_INSTALL_DIR='$($script:Target)'; `$env:GHOSTDRAFT_SKIP_PATH='1'; & '$($script:InstallScript)'" *> $null
